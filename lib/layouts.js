...
function createIndependentMultilayerLayout(network, state) {
    if (!network) {
    ...
    var nodes = network.nodelayers;
    var layers = network.layers;
    var elements = network.elements;

    console.log("setting ILD to:" + state.interLayerDistance);
    Options.interLayerDistance = state.interLayerDistance;
    console.log("Result: " + Options.interLayerDistance);
    ...
    for (var i=0;i<layers.length;i++) {
        physicsSimulators[i] = createSimulator(state.physicsSettings || Defaults.physicsSettings);
    }


    var interLayerDistance = state.interLayerDistance;
    ...
    var api = {
        ...
        getSettings: function () {
            return state.physicsSettings || Defaults.physicsSettings;
        },
    ...
    function listenToEvents() {
        //network.on('changed', onGraphChanged);
        for (var i=0;i<physicsSimulators.length;i++) {
    ...
    function getLayer(nodeId) {
        var node = elements['nl' + nodeId];
        var layerIDs = R.map(function (i) { return i.data['id'] }, layers);
        return layerIDs.indexOf('l' + node.data['layer']); // Return the index of the layer of node
    }

    function initLayers() {
        nodes.forEach(function (node) {
            var layer = getLayer(node.data.id);
            initBody(node.data.id, layer);
        });
        edges.forEach(initLink);
    }
    ...
    function initBody(nodeId, layer) {
        ...
        nodeBodies['nl' + nodeId] = body;
        updateBodyMass('nl' + nodeId);
    ...
    function initLink(link) {
        updateBodyMass(link.data['source']);
        updateBodyMass(link.data['target']);


        // Only add springs for intra-layer links
        var fromLayer = elements[link.data['source']].data['layer'];
        var toLayer = elements[link.data['target']].data['layer'];
        var layerIndex = getLayer(link.data['source'].substring(2));
        if ( toLayer === fromLayer ) {
            var fromBody = nodeBodies[link.data['source']],
                toBody  = nodeBodies[link.data['target']],
    ...
    function updateBodyMass(nodeId) {
        //console.log(nodeId.substring(2));
        var body = nodeBodies[nodeId];
    ...
    function nodeMass(nodeId) {
        ...
        // This is faster but will not update when edges are added
        if (node.data.degree) {
            mass = node.data.degree;
        } else {
            for (var i = 0; i < edges.length; i++) {
                var to = edges[i].data['target'];
                var from = edges[i].data['source'];
                if (to === id || from === id) mass++;
            }
        }
        if (mass === 0) return 1;
        return 1 + Math.log(mass);//mass / 4.0;
    }

    function getSettings() {
        return Defaults.physicsSettings;
    }
...
function createConnectedMultilayerLayout(network, state) {

    var createSimulator = require('ngraph.physics.simulator');
    var physicsSimulator = createSimulator(state.physicsSettings);
    ...
    for ( i=0; i < nodes.length; i++ ) {
        ...
        var z = layers.indexOf('l' + nodes[i].data['layer']) * Options.interLayerDistance;
        elements['nl' + nodes[i].data.id].z = z;
    }
    ...
    var api = {
        ...
        setInterLayerDistance: function ( dist ) {
            Options.interLayerDistance = dist;
        },
    ...
    function nodeMass(nodeId) {
        ...
        // This is faster but will not update when edges are added
        if (node.data.degree) {
            mass = Math.max.apply(0, node.data.degree);
        } else {
            for (var i = 0; i < edges.length; i++) {
                var to = edges[i].data['target'];
                var from = edges[i].data['source'];
...
function createManualLayout( network, settings ) {
    ...
    //var max = 150;
    //var min = -150;
    //network.nodelayers.forEach(function (n) {
    //    var nl = elements['nl' + n.data.id];
    //    nl.position = {};
    //    nl.position.x = Math.floor(Math.random() * (max - min + 1)) + min;
    //    nl.position.y = Math.floor(Math.random() * (max - min + 1)) + min;
    //    nl.position.z = Math.floor(Math.random() * (max - min + 1)) + min;
    //});
    ...
    return {
        ...
        getNodePosition: function (node) {
            ...
            return node.position;//network.nodelayers[id].position;
        },
        nodeUIBuilder: Defaults.nodeUIBuilder,
        linkUIBuilder: Defaults.linkUIBuilder,
        nodeRenderer: Defaults.nodeRenderer,
...
// TODO: Test this!!
function createCircleLayout( network, state ) {
    var n = network.nodes.length;
    var C = 8 * n * state.nodesize;
...
exports.circle = createCircleLayout;
