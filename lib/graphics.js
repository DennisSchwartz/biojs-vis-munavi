...
function Graphics ( state ) {
    ...
    function run () {
        ...
        if (state.zoomingEnabled) state.controls.update();
    ...
    function createCamera ( state ) {
        ...
        camera.position.x = state.cameraPosition.x;
        camera.position.y = state.cameraPosition.y;
        camera.position.z = state.cameraPosition.z;
        state.camera = camera;
    ...
    function renderFrame() {
        ...
        if ( state.directed ) {
            ...
            edges.forEach(function ( edge ) {
                ...
                rainbow.setSpectrumByArray(['00ff00', 'ff0000']);
                var colorKey = Math.floor((Math.log(edge.data.weight) / Math.log(state.elements.maxWeight)) * 100);
                var color = rainbow.colourAt(colorKey);
                //console.log(color);
...
function orderLayers(state) {
    if (!state.layerOrder) return state.elements;

    var network = state.elements;
    var oldOrder = network.layers;
    var newOrder = [];
    for (var i = 0; i < state.layerOrder.length; i++) {
        for (var k = 0; k < oldOrder.length; k++) {
            if (oldOrder[k].data.id === 'l' + state.layerOrder[i]) {
    ...
    //console.log("HELLO!");
    //console.log(state.layerOrder);
    console.log(newOrder, oldOrder);
    network.layers = newOrder;
    return network;
}
