...
function Munavi (el, state) {
    this.el = el;
    //var merged = Utils.merge(Defaults.state, state);
    this.state = state;
}
/*
 ...
 */
Munavi.prototype.init = function () {
    ...
    // Set container
    if (!this.state.container) this.state.container = this.el;
    if (!this.state.id) this.state.id = this.el.id;
    if (this.state.elements.maxWeight) this.state.maxWeight = this.state.elements.maxWeight;
     ...
     */
    console.log("Creating visualisation with state: ", this.state);
...
function createVisView ( state ) {
    ...
    if ( state.directed ) {
        state.graphics.setNodeUI(function (node) {
            ...
            if (node.data.pathways) {
                var pathwayString = node.data.pathways.join();
                if (pathwayString.indexOf('WNT/Wingless') > -1 &&
                    pathwayString.indexOf('Hedgehog') === -1) {
                    color_ = 0x00ff00;
                }
                if (pathwayString.indexOf('WNT/Wingless') > -1 &&
                    pathwayString.indexOf('Hedgehog') > -1 ) {
                    color_ = 0xff0000;
                    console.log(node.data.pathways);
                }
                if (pathwayString.indexOf('WNT/Wingless') === -1 &&
                    pathwayString.indexOf('Hedgehog') > -1 ) {
                    color_ = 0x0000ff;
                ...
                for (var i = 0; i < state.elements.nodes.length; i++) {
                    if ('n' + node.data['node'] === state.elements.nodes[i].data.id) {
                        color_ = Defaults.nodeColors[i % Defaults.nodeColors.length];
                        state.elements.nodes[i].color = color_;
                    }
                }
            }
    ...
    if ( state.style.layerLabels ) {
        state.elements.layers.forEach(function (l) {
            var sprite = makeTextSprite(l.data.id.substring(1), { fontsize: 18, labelColor: state.style.labelColor });
    ...
    // Add color key
    if (state.style.colorKey) {
        makeColorKey(state.elements, state.visEl, state.style.key);
    }
...
function makeColorKey(network, container, key) {
    var length = 100;
    var colors = [];
    var names = [];
    if (key) {
        ...
        network.nodes.forEach(function(n){
            colors.push("#" + n.color.toString(16));
            names.push(n.data.id.substr(1))
        });
    }
    ...
    for ( var i = 0; i < colors.length; i++ ) {
        ...
        context.beginPath();
        context.arc(25, 25 * (i + 1), 10, 0, 2 * Math.PI);
        context.fill();//network.nodes[i].color);
        context.fillText(names[i], 50, 25 * (i + 1));
    }
...
function makeTextSprite( message, parameters )
{
    ...
    // text color
    context.fillStyle = parameters.labelColor;
