...
function createVisView ( state ) {
    ...
    if (state.style.colorKey) {
        makeColorKey(state.elements, state.visEl);
    }
...
function makeColorKey(network, container) {
    var fontface = 'Helvetica';
    var fontsize = '10';
    var canvas = document.createElement('canvas');
    container.appendChild(canvas);
    canvas.height = 50 * network.nodes.length;
    canvas.width = 100;
    ...
    for ( var i = 0; i < network.nodes.length; i++ ) {
        context.fillStyle = "#" + network.nodes[i].color.toString(16);
        context.font = fontsize + "px " + fontface;
        context.beginPath();
        context.arc(25, 25 * (i + 1), 10, 0, 2 * Math.PI);
        context.fill();//network.nodes[i].color);
        context.fillText(network.nodes[i].data.id.substr(1), 50, 25 * (i + 1));
    }
